        ...
        {Object.entries(positions).map(([jobId, pos]) => {
          ...
          return (
            <div
              ...
              className={`rounded-lg border px-3 py-2 flex flex-col justify-between cursor-pointer transition-all duration-200 select-none ${getStatusBorderClass(job.status, isSelected)}`}
            >
              ...
              <div className="flex justify-between items-center text-[10px] font-mono text-github-textMuted mt-1">
                ...
                <span>{job.passedTests} / {job.totalTests} passed</span>
              </div>
              {job.status === "running" && (
                <div className="absolute bottom-0 left-0 right-0 h-1 bg-github-border rounded-b-lg overflow-hidden">
                  <div style={{ width: `${job.progress}%` }} className="h-full bg-github-warning transition-all duration-300" />
                </div>
              )}
            </div>
          );
